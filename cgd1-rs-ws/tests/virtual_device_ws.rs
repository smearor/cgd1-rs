...
fn isolate_token_dir() -> PathBuf {
    let dir = temp_dir().join(format!("cgd1_ws_test_{}", std::process::id()));
...
fn pre_generate_token() {
    ...
    let mac: MacAddress = VIRTUAL_MAC.parse().unwrap();
    let _ = store.load_or_generate(&mac);
}

/// Send a WS request and read the next response.
async fn send_request(ws: &mut WebSocketStream<MaybeTlsStream<TcpStream>>, id: u32, command: Value) -> Value {
    ...
    loop {
        let msg = ws.next().await.expect("should receive a response").expect("response should be Ok");
        match msg {
            Message::Text(text) => {
...
fn assert_ok(response: &Value, id: u32) -> &Value {
    assert_eq!(response["id"].as_u64(), Some(id as u64), "response id should match request id");
    assert!(
        response.get("error").is_none() || response["error"].is_null(),
        "response should not have an error: {response}"
    );
    response.get("result").expect("response should have a result field")
}
...
async fn ws_full_virtual_device_flow() {
    ...
    let state = ServerState::new(Backend::Virtual).await.expect("server state creation should succeed");
    ...
    let (mut ws, _) = connect_async(&ws_url).await.expect("WebSocket connection should succeed");

    // 1. Scan
    let scan_resp = timeout(Duration::from_secs(5), send_request(&mut ws, 1, json!({ "type": "scan", "duration_secs": 1 })))
        .await
    ...
    assert_eq!(connect_result["connected"].as_bool(), Some(true), "device should be connected");

    // 3. SyncTime
    let sync_resp = timeout(Duration::from_secs(5), send_request(&mut ws, 3, json!({ "type": "sync_time", "address": VIRTUAL_MAC })))
        .await
        .expect("sync_time should not time out");
    let sync_result = assert_ok(&sync_resp, 3);
    assert_eq!(sync_result["synced"].as_bool(), Some(true), "time should be synced");

    // 4. ReadFirmware
    let firmware_resp = timeout(Duration::from_secs(5), send_request(&mut ws, 4, json!({ "type": "read_firmware", "address": VIRTUAL_MAC })))
        .await
        .expect("read_firmware should not time out");
    let firmware_result = assert_ok(&firmware_resp, 4);
    assert!(firmware_result["firmware"].as_str().is_some(), "firmware response should contain a firmware string");

    // 5. ReadBattery
    let battery_resp = timeout(Duration::from_secs(5), send_request(&mut ws, 5, json!({ "type": "read_battery", "address": VIRTUAL_MAC })))
        .await
        .expect("read_battery should not time out");
    let battery_result = assert_ok(&battery_resp, 5);
    assert!(battery_result["battery"].as_u64().is_some(), "battery response should contain a battery level");

    // 6. ReadSettings
    let settings_resp = timeout(Duration::from_secs(5), send_request(&mut ws, 6, json!({ "type": "read_settings", "address": VIRTUAL_MAC })))
        .await
        .expect("read_settings should not time out");
    let settings_result = assert_ok(&settings_resp, 6);
    assert!(settings_result.get("volume").is_some(), "settings response should contain volume");

    // Clean up.
    ws.close(None).await.ok();
    server_handle.abort();

    let _ = remove_dir_all(&_token_dir);
}
