...
fn isolate_token_dir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("cgd1_ws_test_{}", std::process::id()));
...
async fn ws_full_virtual_device_flow() {
    ...
    let (mut ws, _) = tokio_tungstenite::connect_async(&ws_url).await.expect("WebSocket connection should succeed");

    // 1. Scan
    let scan_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 1, json!({ "type": "scan", "duration_secs": 1 })))
        .await
    ...
    // 2. Connect
    let connect_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 2, json!({ "type": "connect", "address": VIRTUAL_MAC })))
        .await
    ...
    // 3. SyncTime
    let sync_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 3, json!({ "type": "sync_time", "address": VIRTUAL_MAC })))
        .await
    ...
    // 4. ReadFirmware
    let firmware_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 4, json!({ "type": "read_firmware", "address": VIRTUAL_MAC })))
        .await
    ...
    // 5. ReadBattery
    let battery_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 5, json!({ "type": "read_battery", "address": VIRTUAL_MAC })))
        .await
    ...
    // 6. ReadSettings
    let settings_resp = tokio::time::timeout(Duration::from_secs(5), send_request(&mut ws, 6, json!({ "type": "read_settings", "address": VIRTUAL_MAC })))
        .await
